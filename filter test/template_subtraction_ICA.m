...
template1 = chn_data;
X = chn_data(temp, 1:period_avg+prebuffer);
ricaModel = rica(X', numComponents, 'NonGaussianityIndicator', ones(1, numComponents));
...
S = transform(ricaModel, X');

weights = sum(abs(W), 1);
[~, artifactidx] = min(weights);
S(:,artifactidx) = 0;
template = S*W';
template1(temp, 1:period_avg+prebuffer) = template';
for i = 1:NSTIM
...
for i = 1 : NSTIM
    segment = (-prebuffer+1 + trigs(i) ):min(period+ trigs(i), length(amplifier_data) );
    amplifier_data_copy( segment) = amplifier_data_copy( segment) - template1(i,1:length(segment));
