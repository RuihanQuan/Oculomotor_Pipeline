...
duration = str2double(durations{temp(n)});
freq = str2double(frequencies{temp(n)});
region = regions(temp(n), :);
cur = str2double(currents{temp(n)});
...
% %% varying regions of stim
label = {['trial # ' num2str(trial_num(1)) ' stim at ' region{1} ], ['trial # ' num2str(trial_num(2)) ' stim at ' region{2}]};
coloring = {'b', 'r'};
fig = figure;
tiledlayout(5,1)
nexttile;
...
xline(duration, 'k--', 'DisplayName',[num2str(duration) ' ms'])
hold off
box off
axis([-50 segs.timeframe(end) -30 10]);
title(sprintf("Average Eye horizontal Position stim with %i uA at %i Hz in %i ms",cur, freq, duration),'Fontsize',12);
legend
xlabel("time (ms)")
...
title(sprintf("Average Eye horizontal Velocity stim with %i uA at %i Hz in %i ms",cur, freq, duration),'Fontsize',12);
legend
xlabel("time (ms)")
...
if ~isempty(duration)
    rectangle('Position', [0, 0, duration, length(ua)+1], 'FaceColor', 'yellow', 'FaceAlpha', 0.2,  'EdgeColor', 'none');
end
            % Formatting
...
ylabel('trials');
yticks(1:length(ua));
title(sprintf("Raster Plot stim with %i uA at %i Hz in %i ms " + label{n},cur, freq, duration),'Fontsize',16);
grid off;
box off;
hold off;
ax = gca;
...
legend
title(sprintf("Average Firing Rate for one Cell stim with %i uA at %i Hz in %i ms",cur, freq, duration),'Fontsize',12);
xlabel("time (ms)")
...
hold on
scatter(xcoords(stim_channels1), ycoords(stim_channels1), 'red', 'filled','LineWidth',3,'DisplayName', ['Stim Channels ' 'at ' region{1} ])
